...
const playTrack = async ({token, track, device_id}) => {
    const options = {
        method: 'PUT',
        body: JSON.stringify({
            uris: [track],
    ...
    return spotifyFetch({url: SPOTIFY_ENDPOINTS.PLAY, options, token, device_id});
}
...
const initializeSpotifyPlayer = ({accessToken, readyCallback}) => {
